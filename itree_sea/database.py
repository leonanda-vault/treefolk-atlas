...
def get_coefficients(
    ...
    try:
        ...
        if scientific_name:
            ...
            row = cur.fetchone()
            if row:
                return _row_to_coefficients(row, match_level="species")
        ...
        if genus_query:
            ...
            row = cur.fetchone()
            if row:
                return _row_to_coefficients(row, match_level="genus")
...
def _row_to_coefficients(row: sqlite3.Row, match_level: str) -> AllometricCoefficients:
    """Convert a database row to an AllometricCoefficients dataclass."""
